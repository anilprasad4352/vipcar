...
@IonicPage()
@Component({
  selector: 'page-carwash',
  templateUrl: 'carwash.html',
})
...
list1: any;
 ...
 this.langdata=JSON.parse(localStorage.getItem("langdata"));
  ...
  loading.present();
    ...
    this.http.get('http://preferwork.com/apiproject/api/getProduct/carwash/'+this.navParams.get('pid'), this.loginv, headers)
  .then(data => {
...
homeGo() {
 this.navCtrl.setRoot(HomePage);
  ...
  productMove(pid) {	 
	  this.navCtrl.setRoot(BuyPage, {pid: pid,table:'carwash'});
  }
    loadMap(t,g,loc) {
